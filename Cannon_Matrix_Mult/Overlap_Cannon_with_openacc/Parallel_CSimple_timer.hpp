...
class CSimple_timer {
  //we need to access this in main 	
 //long long int comp_time;
...
static long long int print_timing_results(int some_rank, int comm_size, MPI_Comm comm1) {
    ...
    */
    ...
    if (some_rank == 0){
      ...
      */
      ...
      std::cout << scope_names[0] << " " << max[0]*table["Total"].num_calls << std::endl;

      //print the computation time 
      std::cout << scope_names[1] << " " << max[1]*table["comp"].num_calls << std::endl;

      //the communication time is the difference
      std::cout << "comm time: " << max[0]*table["Total"].num_calls - (max[1]*table["comp"].num_calls) << std::endl;
      
      //initialize the class member for the maximum computation time
      return max[1]*table["comp"].num_calls;
